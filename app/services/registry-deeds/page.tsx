...
import {
  FileSignature, Gift, FileKey, SearchCheck, Landmark, CopyCheck,
  FileCheck2, BadgeCheck, Phone
} from "lucide-react";
...
const cardIcons = [FileSignature, Gift, FileKey, SearchCheck, Landmark, CopyCheck];
const featureIcons = [FileCheck2, BadgeCheck, SearchCheck, CopyCheck];

export default function RegistryDeedsPage() {
  const { isUrdu, t } = useLanguage();
  const sp = t.servicePages["registry-deeds"];

  const features = sp.whyFeatures.map((f, i) => ({
    icon: featureIcons[i] || FileCheck2,
    title: f.title,
    text: f.text,
  }));
  ...
  return (
    <>
      ...
      <div className="border-b border-blue-100 dark:border-blue-950/40 bg-blue-50/80 dark:bg-blue-950/25 transition-colors duration-200">
